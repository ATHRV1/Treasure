from straw_hat1 import *
from heap1 import *
from treasure1 import *
import random
import time
...
def main(n=10**3, m=50):
    ...
    print("-" * 54)
    for n in n_values:
        print(f"| add_treasure() | n = {str(n).ljust(7)} | m = 1048600 | ", end="")
        test_treasury = StrawHatTreasury(1048600)
        last_added = 0
        for i in range(n):
    ...
    for n in n_values:
        test_treasury = StrawHatTreasury(1048600)
        last_added = 0
        print(f"| get_completion_time() | n = {str(n).ljust(7)} | m = 1048600 | ", end="")
        for i in range(n):
            r = random.randint(1, 5)
            test_treasury.add_treasure(Treasure(i, 10 ** 9, last_added + 10 ** 9 * r))
            last_added += 10 ** 9 * r
    ...
    print("-" * 62)
    for m in m_values:
        print(f"| add_treasure() | n = 1048600 | m = {str(m).ljust(8)} | ", end="")
        test_treasury = StrawHatTreasury(m)
        last_added = 0
        for i in range(1048600):
            r = random.randint(1, 5)
            test_treasury.add_treasure(Treasure(i, 10 ** 9, last_added + 10 ** 9 * r))
            last_added += 10 ** 9 * r
        start_time = time.perf_counter_ns()
    ...
    print("-" * 62)
    for m in m_values:
        print(f"| get_completion_time() | n = 1048600 | m = {str(m).ljust(8)} | ", end="")
        test_treasury = StrawHatTreasury(m)
        last_added = 0
        for i in range(1048600):
            r = random.randint(1, 5)
